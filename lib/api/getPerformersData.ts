...
export async function getPerformersData(data: PagingDataProps) {
  ...
  try {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/Performers?pageNum=${pagingData?.num || 1}&pageSize=${pagingData?.size || 10}`, {
      method: "GET",
      headers: {
